...
//TODO: make this file general header
#include "Version.h"
...
namespace{
cl::opt<bool>
IgnoreExternalFunctions("ignore-ext-func",
		cl::desc("Ignore calls to external functions, assume they have no impact on constraints (default=true)"),
		cl::init(true));
cl::opt<bool>
IgnoreNonInitVar("ignore-ninit-var",
		cl::desc("Ignore non-initialized variables,assume they will not alias (default=true)"),
		cl::init(true));
}
char Andersens::ID = 0;
...
AliasAnalysis::AliasResult Andersens::alias(const Location &L1,
		const Location &L2) {
	Node *N1 = &GraphNodes[FindNode(getNode(const_cast<Value*>(L1.Ptr)))];
	Node *N2 = &GraphNodes[FindNode(getNode(const_cast<Value*>(L2.Ptr)))];
...
Andersens::getModRefInfo(ImmutableCallSite CS, const Location &Loc) {
	...
	if (const Function *F = CS.getCalledFunction())
		if (F->isDeclaration()) {
			Node *N1 = &GraphNodes[FindNode(getNode(const_cast<Value *>(Loc.Ptr)))];
...
bool Andersens::AddConstraintsForExternalCall(CallSite CS, Function *F) {
	...
	if (F->getName() == "atoi" || F->getName() == "atof" ||
			...
			F->getName() == "modf" ||
			// Just temperory fix
			//TODO: make a principal approach!
			F->getName() == "pthread_mutex_lock" || F->getName() == "pthread_mutex_unlock" ||
			(F->getName().find("pthread") != StringRef::npos &&
					(F->getName().find("_init") != StringRef::npos)) )
		return true;
	...
	//return false;
	return IgnoreExternalFunctions;
}
...
bool Andersens::AnalyzeUsesOfFunction(Value *V) {
	...
	for (Value::use_iterator UI = V->use_begin(), E = V->use_end(); UI != E; ++UI)
		if (isa<LoadInst>(*UI)) {
			...
			CallSite CS(CI);
			// This is temperory fix for functions that are called by pthread_create
			// Right now the mechanism makes argument of all this functions to point
			// to universal set (and thus alias). but we know that pthread_create does
			// not do any harm to the arguments. we treat this situation as a function
			// call later on.
			// TODO: make a principal approach!
			if (CI->getCalledFunction()->getName() == "pthread_create" && CS.getArgument(2) == V)
				return false;
